...
#define SHADOW_HEADER_SIZE      (sizeof(EFI_PHYSICAL_ADDRESS) * 2)
  ...
  )
{
  EFI_STATUS              Status;
  VOID                    *Memory;
  EFI_PHYSICAL_ADDRESS    ShadowMemory;
  UINTN                   PageNumber;
  ...
  if (mCpu != NULL) {
    //
    // Add space for exchanging memory address information.
    //
    AllocationSize += SHADOW_HEADER_SIZE;
    PageNumber = EFI_SIZE_TO_PAGES(AllocationSize);

    //
    // Turn to page allocation
    //
    Memory = InternalAllocatePages(MemoryType, PageNumber);

    //
    // Ask CPU driver to shawdow the allocated pages.
    //
    ...
    if (!EFI_ERROR(Status)) {
      //
      // Cpu driver will pass the shadowed memory address at the first 8-byte.
      //
      ShadowMemory = *(EFI_PHYSICAL_ADDRESS *)Memory;
      //
      // Replace it with the original physical address of allocated memory.
      //
      *(EFI_PHYSICAL_ADDRESS *)Memory = (EFI_PHYSICAL_ADDRESS)Memory;
      //
      // Save the allocated memory size in the second 8-byte.
      //
      *((EFI_PHYSICAL_ADDRESS *)Memory + 1) = EFI_PAGES_TO_SIZE(PageNumber);
      //
      // Return the shadowed memory address, skipping the first 8-byte.
      //
      Memory = (UINT8 *)ShadowMemory + SHADOW_HEADER_SIZE;
    }
  } else {
  ...
  )
{
  ...
  if (mCpu != NULL && (EFI_PHYSICAL_ADDRESS)Buffer > 0x100000000ull) {
    //
    // Get the original physical memory address and size before releasing
    // the shadowed memory.
    //
    Buffer = (UINT8 *)Buffer - sizeof(EFI_PHYSICAL_ADDRESS);
    Length = *(EFI_PHYSICAL_ADDRESS *)Buffer;

    Buffer = (UINT8 *)Buffer - sizeof(EFI_PHYSICAL_ADDRESS);
    Memory = *(EFI_PHYSICAL_ADDRESS *)Buffer;
